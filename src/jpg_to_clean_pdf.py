...
def exec_mono(pdf_path_list):
    for pdf_path in pdf_path_list:
        print(pdf_path)
        jpg_dir_path = join(
            os.path.dirname(pdf_path), PDF_TO_JPG_DIR, basename(pdf_path)
        )
        ...
        with utils.timer("jpgファイルをpdf化"):
            clean_pdf_path = join(
                os.path.dirname(pdf_path), CLEAN_PDF_DIR, basename(pdf_path)
            )
            jpg_to_pdf(new_jpg_dir_path, clean_pdf_path)


def flow(jpg_dir_path, pdf_path):
    devide_jpgs(jpg_dir_path, norm_img_size=False)
...
def exec_multi(pdf_path_list):
    ...
    for pdf_path in pdf_path_list:
        ...
        if os.path.isdir(jpg_dir_path):
            new_df_list.append(p.apply_async(flow, args=[jpg_dir_path, pdf_path]))

    ready_list = [int(r.ready()) for r in new_df_list]
    while sum(ready_list) != len(ready_list):
        print(sum(ready_list), len(ready_list))
        ready_list = [int(r.ready()) for r in new_df_list]
        sleep(3)
...
if __name__ == "__main__":
    ...
    exec_mono(pdf_path_list)
    # exec_multi(pdf_path_list)
