...
RE_BALLOON_NG_MATCH = re.compile(
    '(^[0-9]{2,}$|^[a-zA-Z()\-\\\/%:;"\'\.,_"]{1,}$|^フ$|^℃$|^ù$)', re.UNICODE
)
...
class Balloons(Koma):
    ...
    def _define_rect(self, positions):
        ...
        rect = [
            lt.get("x", 0),
            lt.get("y", 0),
            rt.get("x", 0) - lt.get("x", 0),
            lb.get("y", 0) - lt.get("y", 0),
        ]
        return rect
    ...
    def define_balloon(self, text_annotations):
        ...
        ta_list = [
            self.get_ta_value(ta) for ta in text_annotations[1:] if self.is_good_ta(ta)
        ]
    ...
    def is_good_ta(self, text_annotation):
        ...
        if text not in SKIP_LONG_WIDTH_LIST:
            if area_width > area_height * self.NG_LONG_WIDTH_RATIO:
                ...
                print(
                    "{} > {} * {}".format(
                        area_width, area_height, self.NG_LONG_WIDTH_RATIO
                    )
                )
                return False
