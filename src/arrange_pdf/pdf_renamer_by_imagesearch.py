...
def fetch_item_info(page1_path):
    ...
    if elem_imgbox:
        driver.execute_script("window.scrollTo(0, {});".format(elem_imgbox.location['y']))
        ss_bio = BytesIO(driver.get_screenshot_as_png())
        screen_shot_color = cv2.imdecode(np.asarray(bytearray(ss_bio.read()), dtype=np.uint8), 1)
        ...
        match_result = cv2.matchTemplate(screen_shot_gray, img_template, cv2.TM_CCOEFF_NORMED)
        ...
        # 検出領域を四角で囲んで保存(確認用)
        rect_result = cv2.rectangle(screen_shot_color, top_left, bottom_right, (0, 0, 255), 4)
        ...
        print('elem_imgbox.size[width], x_elem_loc:', elem_imgbox.size['width'], x_elem_loc)
    ...
    while target is None:
        ...
        a_list = [a for a in driver.find_elements_by_css_selector("a.irc_vpl") if a.text == '表示']
        if a_list:
        ...
        try:
            ...
            # 最良結果を使うパティーン
            best_result = driver.find_elements_by_xpath("//div[contains(text(), 'この画像の最良の推測結果')]")
            if best_result:

                driver.get(best_result[0].find_element_by_tag_name('a').get_attribute('href'))
            amazon_a_list = driver.find_elements_by_xpath("//a[contains(@href, 'amazon')]")
            # 商品へのリンクであるものに絞る
            amazon_a_list = [a for a in amazon_a_list if 'dp' in a.get_attribute('href')]
            if len(amazon_a_list):
                amazon_a_list[0].click()
                sleep(0.1)
                title = driver.find_element_by_id('productTitle').text
                try:
                    author = driver.find_element_by_css_selector('span.author').text[:-4]
                except:
                    author = ''
                return author, title
            else:
    ...
    for pdf_path in pdf_path_list[:]:
        ...
        if filename:
            ...
            if os.path.exists(os.path.join(pdf_dir, filename)):
                ...
                print('rename:{} -> {}'.format(pdf_path, os.path.join(pdf_dir, filename)))
        else:
            print('no result')
